function initNavigation() {
    ...
    navBtns.forEach(btn => {
        ...
        if (tab === 'grammar' && !levelSystem.isUnlocked(level.name, 'grammar_easy')) btn.classList.add('locked');
    });
    navBtns.forEach(btn => {
...
async function displayWord(word) {
    ...
    if (!data || data.t[0] === word) {
        dictionaryResult.innerHTML = `<div class="word-result" style="text-align:center;padding:40px;"><p style="font-size:24px;font-weight:600;margin-bottom:8px;">${word}</p><p style="color:#c62828;">Слово не найдено</p><p style="color:#8c8c8c;font-size:13px;margin-top:8px;">Попробуйте другое слово</p></div>`;
        return;
    }
    dictionaryResult.innerHTML = `<div class="word-result"><div class="word-header"><span class="word-title">${word}</span>${data.p ? `<span class="word-phonetic">${data.p}</span>` : ''}${data.pos ? `<span style="color:#8c8c8c;font-size:12px;">${data.pos}</span>` : ''}</div><div class="word-translations">${data.t.map(t => `<span class="word-tag">${t}</span>`).join('')}</div>${data.ex.length > 0 ? `<div class="word-examples"><h4>Примеры</h4>${data.ex.map(e => `<div class="word-example"><div class="word-example-en">${e.en}</div><div class="word-example-ru">${e.ru}</div></div>`).join('')}</div>` : ''}</div>`;
    ...
    newAchs.forEach(a => { if (!userData.achievements.includes(a)) userData.achievements.push(a); });
    saveUserData();
    updateLevelDisplay();
}
