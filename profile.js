document.getElementById('profileBtn').addEventListener('click', () => {
    ...
    achContainer.innerHTML = achievementsList.map(a => {
        ...
        return `<div class="achievement-badge${earned ? '' : ' locked'}">
            ${earned ? '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg>' : '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>'}
            ${a.name}
        </div>`;
    }).join('');
    ...
    if (userData.mistakes.length === 0) {
        ...
        mistContainer.innerHTML = userData.mistakes.slice(-10).reverse().map(m => 
            `<div class="mistake-item">${m.word} &rarr; ${m.correct}</div>`
        ).join('');
    }
});
